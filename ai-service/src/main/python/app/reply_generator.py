...
def generate_suggested_reply(complaint_text: str, category: str, priority: str) -> dict:
    ...
    Falls back to a fixed template whenever the API keeps failing or timing
    out, or the output looks like an injection attempt succeeded - an
    agent always gets a usable reply either way. Retries once first, since
    a free-tier API call occasionally fails transiently (a brief timeout or
    a "model overloaded" response) even when the service is healthy.
    """
    ...
    for attempt in range(MAX_ATTEMPTS):
        try:
            reply = _call_gemini(complaint_text, category, priority, similar)
